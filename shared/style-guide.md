...
A skill is a folder under one of the category dirs (`skills/personas/`, `skills/capabilities/`, `skills/stacks/`). The folder name **must equal** the `name:` field, lowercase-kebab, no leading/trailing/consecutive hyphens, 1–64 chars. Inside lives one `SKILL.md`.
...
4. **State anti-triggers when overlap is likely.** "Not for code review of a single file — see `senior-code-reviewer`."
...
7. **Anti-patterns** — what this skill does NOT do, and the kinds of mistakes a naive impersonation would make.
8. **Handoffs** — when and how to defer to another skill in the library. Reference by skill name in backticks.
9. **Quick reference** — a compressed cheat sheet at the bottom for fast scanning on re-activation.
