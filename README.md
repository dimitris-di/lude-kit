...
The scripts symlink, so future `git pull`s update your installed skills without re-running anything.
...
Every skill in this library follows the same nine-section structure — Role → When to invoke → Operating principles → Workflow → Deliverables → Quality bar → Anti-patterns → Handoffs → Quick reference — so an orchestrator can scan many of them quickly and so reviewers know exactly what's missing.
